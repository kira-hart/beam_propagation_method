...
E0 = zeros(NX,1);
for x=1:NX
	E0(x) = GaussianBeam1DRotated( cx(x)+ cx(NX)*0.4, 0.0, w0, k0, rangle);
end;
...
for k=1:zstop/zstep
  ...
  ac = 3/8.0 +1.0/24.0*sqrt(27 + 2*DZ*DZ);
  bc = 1/12.0*DZ;
...
E1 = zeros(NX,1);
for x=1:NX
	E1(x) = GaussianBeam1DRotated( cx(x) + cx(NX)*0.4, zcoor, w0, k0, rangle);
end
