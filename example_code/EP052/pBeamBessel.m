...
% physics  = Bessel beam approximation formation behind axicon
...
for x=1:NX
  for y=1:NX
     am0(x,y) = exp( -(cx(x)^2 + cx(y)^2)/w0^2 )*Mask_Axicon(0.25/360*2*pi, k0, cx(x), cx(y));
  end
end
...
figure(6);
imagesc(cx,zs,holder); colorbar;
xlabel('transverse location');
ylabel('propagation distance');
xlim([-0.0045,0.0045]);